...
select *  from response r INNER JOIN question q ON q.id = r.question_id WHERE q.survey_id = 15 AND r.response_hash != ''  GROUP BY r.response_hash, q.index, r.id, q.id ORDER BY q.index ASC;